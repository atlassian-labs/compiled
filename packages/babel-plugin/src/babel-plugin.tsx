...
import { transformCssCallExpression, transformCssTaggedTemplateExpression } from './css';
import { transformCssProp } from './css-prop';
import { transformKeyframesCallExpression } from './keyframes';
import { transformStyledCallExpression, transformStyledTaggedTemplateExpression } from './styled';
    ...
    visitor: {
      Program: {
        ...
        exit(path, state) {
          ...
          path.addComment(
            'leading',
            ` ${parseFilename(state.filename)} generated by ${packageJson.name} v${version} `
          );
      ...
      TaggedTemplateExpression(path, state) {
        if (isCompiledCSSTaggedTemplateExpression(path.node, state)) {
          transformCssTaggedTemplateExpression(path, { context: 'root', state, parentPath: path });
          return;
        }
        // if (
        ...
        if (isCompiledStyledTaggedTemplateExpression(path.node, state)) {
          transformStyledTaggedTemplateExpression(path, {
            context: 'root',
            state,
            parentPath: path,
          });
      ...
      CallExpression(path, state) {
        if (isCompiledCSSCallExpression(path.node, state)) {
          transformCssCallExpression(path, { context: 'root', state, parentPath: path });
          return;
        }

        if (isCompiledKeyframesCallExpression(path.node, state)) {
          transformKeyframesCallExpression(path, { context: 'root', state, parentPath: path });
          return;
        }

        if (isCompiledStyledCallExpression(path.node, state)) {
          transformStyledCallExpression(path, { context: 'root', state, parentPath: path });
          return;
        }
      },
      ...
      JSXOpeningElement(path, state) {
        ...
        transformCssProp(path, { context: 'root', state, parentPath: path });
      },
    },
  };
});
