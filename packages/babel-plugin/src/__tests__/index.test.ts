...
describe('babel plugin', () => {
  ...
  it('should not generate dynamic styles with a static concat call', () => {
    const code = `
      ...
      const styles = css({ color: "red ".concat('var(--ds-red)') });

      export default () => <div css={styles}><label /></div>;
    `;

    const actual = transform(code);
    expect(actual).not.toMatch(/color:\s*var\(--_/);
    expect(actual).not.toMatch(/style=\{\{/);
    expect(actual).toInclude('._syaz1d8o{color:red var(--ds-red)}');
    expect(actual).toMatchInlineSnapshot(`
      "/* File generated by @compiled/babel-plugin v0.0.0 */
      import * as React from "react";
      import { ax, ix, CC, CS } from "@compiled/react/runtime";
      const _ = "._syaz1d8o{color:red var(--ds-red)}";
      const styles = null;
      export default () => (
        <CC>
          <CS>{[_]}</CS>
          {
            <div className={ax(["_syaz1d8o"])}>
              <label />
            </div>
          }
        </CC>
