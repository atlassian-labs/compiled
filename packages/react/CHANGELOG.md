...
- 959dcbe: Fix a compatibility regression in runtime style bucketing for compound pseudo-selectors.
...
- 11c107b: Fix runtime style bucketing for class names generated with `collisionResistantHash`.
...
- b4e6d7d: Import the `JSX` namespace explicitly from `react` instead of relying on the global `JSX` namespace.
...
- 60932d8: Introduces `cssMapScoped` — a new **experimental internal** API that produces non-atomic CSS output. It is intentionally not exported from the public `@compiled/react` TypeScript types. Consumers must use `@ts-expect-error` to import it and should only do so with explicit approval from the Compiled team.
...
- 55514fd: Fix `LibraryManagedAttributes` to restore React 18 compatibility and `defaultProps` stripping.
