...
describe('main', () => {
  ...
  it('should run transforms contained in directories with filtered jscodeshift options', async () => {
    ...
    expect(execShPromise).toHaveBeenCalledWith(
      'jscodeshift --parser=tsx --transform=node_modules/@compiled/css-in-js/dist/codemods/emotion-to-compiled/index.ts src/components/Button.tsx'
    );
  });
  ...
  it('should run transforms contained in directories with all jscodeshift options', async () => {
    ...
    expect(execShPromise).toHaveBeenCalledWith(
      'jscodeshift --parser=tsx --extensions=tsx --ignore-pattern=**/*utils* --verbose=2 --cpus=1 --transform=node_modules/@compiled/css-in-js/dist/codemods/emotion-to-compiled/index.ts src/components/**/*.tsx'
    );
  });
});
