...
const fixtureRoot = join(rootPath, 'fixtures/atlaspack-transformer-test-app');
...
it('transforms assets with babel plugin', async () => {
  const atlaspack = getAtlaspackInstance(fixtureRoot);
  const { changedAssets } = await atlaspack.run();
...
it('transforms assets with babel plugin and source map is correct', async () => {
  const atlaspack = getAtlaspackInstance(fixtureRoot, true);
...
it('transforms assets with custom resolve and statically evaluates imports', async () => {
  const customResolveFixtureRoot = join(
    rootPath,
    'fixtures/atlaspack-transformer-test-custom-resolve-app'
  );
  const atlaspack = getAtlaspackInstance(customResolveFixtureRoot);
  const { changedAssets } = await atlaspack.run();
...
it('transforms assets with custom resolver and statically evaluates imports', async () => {
  const customResolverFixtureRoot = join(
    rootPath,
    'fixtures/atlaspack-transformer-test-custom-resolver-app'
  );
  const atlaspack = getAtlaspackInstance(customResolverFixtureRoot);
  const { changedAssets } = await atlaspack.run();
...
it('transforms assets with compiled and extraction babel plugins', async () => {
  ...
  expect(indexJsCode).toMatchInlineSnapshot(`
    "/* index.jsx generated by @compiled/babel-plugin v0.0.0 */ var parcelHelpers = require("@atlaspack/transformer-js/src/esmodule-helpers.js");
...
it('transforms assets with class name compression enabled', async () => {
  const compressingClassNameFixtureRoot = join(
    rootPath,
    'fixtures/atlaspack-transformer-test-compress-class-name-app'
  );
  const atlaspack = getAtlaspackInstance(compressingClassNameFixtureRoot);
  const { changedAssets, bundleGraph } = await atlaspack.run();
