...
- 991cb99: Throw a build error when a selector ends with a dangling combinator, for example `css({ '>': { marginLeft: 10 } })` or `css({ '& +': { ... } })`. Compiled used to emit `.hash > {}`, which browsers reject, so the declarations silently never applied.
